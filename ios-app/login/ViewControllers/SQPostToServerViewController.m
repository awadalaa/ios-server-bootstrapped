//
//  SQPostToServerViewController.h.m
//  login
//
//  Created by Alaa M Awad on 4/1/15.
...
@implementation SQPostToServerViewController
...
- (NSAttributedString *) sendAttributedString {
    NSString *baseString = NSLocalizedString(@"SEND TO TECHNALAA", @"send to Technalaa button text");
...
- (void) sendButtonPressed:(id)sender {
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:nil message:NSLocalizedString(@"Add a caption and send your image now.", @"send image instructions") delegate:self cancelButtonTitle:NSLocalizedString(@"Cancel", @"cancel button") otherButtonTitles:NSLocalizedString(@"Send", @"Send button"), nil];
...
- (void) alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex {
    if (buttonIndex != alertView.cancelButtonIndex) {
        ...
        NSURL *fileURL = [[tmpDirURL URLByAppendingPathComponent:@"technalaa"] URLByAppendingPathExtension:@"igo"];
...
- (void)documentInteractionController:(UIDocumentInteractionController *)controller didEndSendingToApplication:(NSString *)application {
    [[NSNotificationCenter defaultCenter] postNotificationName:SQImageFinishedNotification object:self];
}
